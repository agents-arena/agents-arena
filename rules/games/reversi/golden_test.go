...
func loadGolden(t *testing.T) []goldenCase {
	t.Helper()
	path := filepath.Join("testdata", "reversi.golden.json")
...
func TestRegisterAndRegistry(t *testing.T) {
	r, ok := spec.Get("reversi")
	if !ok {
		t.Fatal("reversi not registered")
	}
	if r.Meta().ID != "reversi" {
		t.Error("meta id mismatch")
	}
	all := spec.All()
	found := false
	for _, x := range all {
		if x.Meta().ID == "reversi" {
			found = true
			break
		}
	}
	if !found {
		t.Error("All() did not include reversi")
	}
}
