...
func loadGolden(t *testing.T) []goldenCase {
	t.Helper()
	path := filepath.Join("testdata", "checkers.golden.json")
...
func TestRegisterAndRegistry(t *testing.T) {
	r, ok := spec.Get("checkers")
	if !ok {
		t.Fatal("checkers not registered")
	}
	if r.Meta().ID != "checkers" {
		t.Error("meta id mismatch")
	}
	all := spec.All()
	found := false
	for _, x := range all {
		if x.Meta().ID == "checkers" {
			found = true
			break
		}
	}
	if !found {
		t.Error("All() did not include checkers")
	}
}
