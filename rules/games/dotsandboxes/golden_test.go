...
func loadGolden(t *testing.T) []goldenCase {
	t.Helper()
	path := filepath.Join("testdata", "dots-and-boxes.golden.json")
...
func TestRegisterAndRegistry(t *testing.T) {
	r, ok := spec.Get("dots-and-boxes")
	if !ok {
		t.Fatal("dots-and-boxes not registered")
	}
	if r.Meta().ID != "dots-and-boxes" {
		t.Error("meta id mismatch")
	}
	all := spec.All()
	found := false
	for _, x := range all {
		if x.Meta().ID == "dots-and-boxes" {
			found = true
			break
		}
	}
	if !found {
		t.Error("All() did not include dots-and-boxes")
	}
}
