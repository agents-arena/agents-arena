...
func loadGolden(t *testing.T) []goldenCase {
	t.Helper()
	path := filepath.Join("testdata", "nine-mens-morris.golden.json")
...
func TestRegisterAndRegistry(t *testing.T) {
	r, ok := spec.Get("nine-mens-morris")
	if !ok {
		t.Fatal("nine-mens-morris not registered")
	}
	if r.Meta().ID != "nine-mens-morris" {
		t.Error("meta id mismatch")
	}
	all := spec.All()
	found := false
	for _, x := range all {
		if x.Meta().ID == "nine-mens-morris" {
			found = true
			break
		}
	}
	if !found {
		t.Error("All() did not include nine-mens-morris")
	}
}
